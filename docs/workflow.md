...
|:---|:---|:---|
| 名称 |是|导出节点名称|
| 服务器地址 |是|ip或域名，例如:https://pipeline.qiniu.com 或 127.0.0.1:7758|
...
| 导出类型 |是|导出的文件存储类型|
| 文件压缩 |是|是否开启文件压缩功能|
| 最大文件保存天数 |是|数据导出在对象存储中的时限，以天为单位，当该字段为0或者为空时，则永久储存|
...
Workflow支持报警管理，您可以填写报警人相关信息和报警指标，如果满足指标条件，您就可以收到报警通知啦！
...
当您的计算任务是手动执行的时候，Workflow提供任务执行管理，您可以看到您的数据运行任务执行的状态（运行中、成功、失败、就绪、已停止），对于停止或失败的任务，您可以对它进行重新执行操作。

![](https://pandora-kibana.qiniu.com/task_batch.png)

## 运行日志监控

workflow提供运行日志监控，您可以选择数据节点（导出节点、流式计算节点、批量计算节点）来查看各节点的运行日志，帮您宏观监控数据的运行情况，及时发现问题。
