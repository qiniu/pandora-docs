...
系统自动统计日志里字段的高频值、低频值、时序平均值、时序最大值、时序最小值等，在搜索结果页，点击具体字段即可看到字段相关的统计信息。详情请阅读 [字段统计分析报表。](#字段统计分析报表)
...
指标：支持对日志计数、字段的去重计数、平均值、中位数、平均值、最大值、最小值、分位数进行统计。去重计数支持 string，long，boolean 字段，其他指标只能选择数值字段。可添加多个指标。

分组字段：支持时间字段与普通字段。分组字段可以添加多个，当分组字段里出现时间字段时，时间字段呈现为 x 轴，即事件首先按照时间粒度聚合，其他分组字段呈现为多系列数据。若分组字段里没有时间字段，则第一个添加的字段呈现为x轴，事件按照第一个添加的字段分组显示。

对于时间字段分组，系统支持自动 interval 或者用户输入 interval 对搜索结果聚合。

![](https://pandora-kibana.qiniu.com/_timestamp.png)

普通字段分组支持 string、long、boolean 字段类型，支持对普通字段分组排序。

1. 可选排序依据，支持按照统计指标、分组字段的值、分组字段的事件总数排序。
2. 可以选择升序或者降序排列。
3. 可选呈现的分组数量。

![](https://pandora-kibana.qiniu.com/fenzu.png)
...
![](https://pandora-kibana.qiniu.com/%E5%9B%BE%E8%A1%A8.png)
![](https://pandora-kibana.qiniu.com/%E5%9B%BE%E8%A1%A8-%E6%8A%98%E7%BA%BF%E5%9B%BE.png)
![](https://pandora-kibana.qiniu.com/%E5%9B%BE%E8%A1%A8%202.png)
![](https://pandora-kibana.qiniu.com/%E6%8A%98%E7%BA%BF%E9%9D%A2%E7%A7%AF%E5%9B%BE%20.png)
...
![](https://pandora-kibana.qiniu.com/barchart1.png)
...
![](https://pandora-kibana.qiniu.com/barchart2.png)
...
![](https://pandora-kibana.qiniu.com/bar_chart3.png)
...
![](https://pandora-kibana.qiniu.com/line_chart.png)
...
1. 饼图适用于不超过10条的分类数据，玫瑰图则适用于分类较多的场景（10-30条数据）。
...
![](https://pandora-kibana.qiniu.com/piechart1.png)
...
![](https://pandora-kibana.qiniu.com/stack.png)

此时两组数据重叠显示，不利于观察。

堆叠区域图：每一个数据集的起点不同，都是基于前一个数据集。用于显示每个数值所占大小随时间或类别变化的趋势线，堆叠起来的区域图在表现大数据的总量分量的变化情况时格外有用。

![](https://pandora-kibana.qiniu.com/areachart.png)
...
![](https://pandora-kibana.qiniu.com/tulichoose.png)
