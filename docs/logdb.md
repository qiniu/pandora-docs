...
实时数据仓库集中存储了从服务器、终端设备、日志收集器等多种途径收集来的日志。在实时数据仓库，我们支持对收集日志进行滚动输出，毫秒级延迟支持用户第一时间获得信息。
同时实时数据仓库支持增设过滤条件，在海量实时日志中筛选核心数据，滚动展示支持数据第一时间反馈。

![](https://pandora-kibana.qiniu.com/%E5%AE%9E%E6%97%B6%E6%97%A5%E5%BF%97.png)

同时实时仓库支持前往工作流进行数据导出，可以支持将实时数据导出到日志仓库、云存储、时序数据库等多种下游数据仓库，与工作流协同工作，将数据可视化与计算模型可视化相结合，
能够从多维度去了解分析数据。
...
日志分析服务最大的优势在于可以对字符串类型的数据进行倒排索引，可以使用搜索引擎的技术对于日志进行索引，并且
支持各种灵活的搜索方式。其中索引的方式包括两种：
